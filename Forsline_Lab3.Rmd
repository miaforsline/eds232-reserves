...
max_cost <- round(max(pu_data$cost), digits = 2)
...
The highest cost value is `r max_cost` in millions of Australian dollars. 
...
```{r}
all_veg <- cellStats(veg_data, "sum", na.rm = TRUE)
which.max(all_veg)
```

Vegetation 12 is the most abundant class. 
...
-   calculate how much of each vegetation feature occurs inside each
    planning unit (aka the **abundance of the features**)
...
-   The `abundance_data` object contains three columns
    ...
    -   the `feature` column contains the name of each feature (derived
        from `names(veg_data)`)

    -   the `absolute_abundance` column contains the total amount of
        each feature in all the planning units

    -   the `relative_abundance` column contains the total amount of
        each feature in the planning units expressed as a proportion of
        the total amount in the underlying raster data.

-   since all the raster cells containing vegetation overlap with the
    planning units, all of the values in the `relative_abundance` column
    are equal to one (meaning 100%)
...
```{r}
median <- round(median(abundance_data$absolute_abundance_km2), digits = 2)
```
...
-   use `eval_feature_representation_summary()`, which requires:
    ...
    -   a conservation problem object with the planning unit and
        biodiversity data

    -   an object representing a solution to the problem (i.e an object
        in the same format as the planning unit data with values
        indicating if the planning units are selected or not)
...
-   the `absolute_held` column shows the total amount of each feature
    held in the solution (i.e. the planning units covered by protected
    areas)

-   and the `relative_held` column shows the proportion of each feature
    held in the solution (i.e. the proportion of each feature's spatial
    distribution held in protected areas).
...
\#\#12. If we set a target of 20% coverage by protected areas, how many
...
\#\#13. Is there a relationship between the total abundance of a feature
and how well it is represented by protected areas (hint:
...
this workshop since it is easier to install

-   the Gurobi solver is much faster than the lpsymphony solver ([see
...
Yes, we can verify that that all of the targets are met because every value in the relative_held column > 0.05. 
...
```{r}
# calculate total cost of the prioritization
s2_cost <- round(eval_cost_summary(p2, s2[, "solution_1"]), digits = 2)
s3_cost <- round(eval_cost_summary(p3, s3[, "solution_1"]), digits = 2)
s4_cost <- round(eval_cost_summary(p4, s4[, "solution_1"]), digits = 2)
```
...
planning units because it has the most planning units. 
...
It most likely costs less to protect highly degraded areas. 
